...
**正确做法：**
调用该 skill，让它的指令真正加载到上下文中并驱动行为。等该 skill 完成其完整流程后，再进入下一步。

⚠️ **关键提醒：当 Skill 工具返回 "Launching skill: xxx" 时，必须：**
1. **停止当前流程** — 不要继续说"现在我将使用 xxx"
2. **阅读完整指令** — skill 的完整 SKILL.md 内容已加载到上下文中，必须仔细阅读
3. **遵循指令行动** — 严格按照加载的指令第一步开始执行
4. **不要假设** — 不要猜测 skill 应该做什么，必须从指令中确认
