...
3. **提出澄清问题** — 持续深入地就需求的各个方面提问，直到双方达成共识；每个问题附上你建议的答案；一次只问一个问题，等得到反馈后再继续；能通过查阅代码库解答的问题，优先查阅代码库而非提问用户；每次使用浏览器/搜索工具后更新 findings
...
- 对于范围适当的项目，持续深入地就需求的各个方面提问，直到双方对要构建的内容达成共识
- **每个问题附上你建议的答案**，让用户可以确认、修改或否决，降低回答门槛
- **一次只问一个问题**，等得到该问题的反馈后再继续下一个；同时提出多个问题会令人困惑，也容易导致部分问题被忽略
- **优先查阅代码库**：如果一个问题可以通过阅读代码、文档或 git 历史来解答，先自己查阅，不要向用户提问已知答案
- 尽可能使用选择题，但开放式问题也可以
- 重点关注理解：目的、约束、成功标准
...
- **一次一个问题** - 不要用多个问题让人不知所措；等得到反馈后再继续下一个
- **每个问题附上建议答案** - 降低回答门槛，让用户可以确认、修改或否决
- **优先查阅代码库** - 能通过阅读代码/文档/git 历史解答的问题，不要向用户提问
